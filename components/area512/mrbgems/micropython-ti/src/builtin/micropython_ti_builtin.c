...
micropython_ti_get_builtin_class_id(
  ...
  for (
    ...
    const MicroPythonTiBuiltinClass *builtin_class =
      &micropython_ti_builtin_classes[class_id];

    const char *builtin_class_name =
      &micropython_ti_builtin_name_pool[builtin_class->name_offset];
...
get_builtin_method_range(
  ...
  *method_count = 0;
  ...
  if (
    class_id == MICROPYTHON_TI_CLASS_NONE ||
    class_id >= micropython_ti_builtin_class_count
  )
    return;
...
static const MicroPythonTiBuiltinMethod *
find_builtin_method_in_range(
  ...
  for (uint16_t index = 0; index < method_count; index++) {
    const MicroPythonTiBuiltinMethod *builtin_method =
      &micropython_ti_builtin_methods[method_start_index + index];
...
const MicroPythonTiBuiltinMethod *
...
const MicroPythonTiBuiltinArgument *
micropython_ti_get_builtin_argument(
  ...
  if (builtin_argument_index >= micropython_ti_builtin_argument_count)
    return NULL;

  return &micropython_ti_builtin_arguments[builtin_argument_index];
}

const char *
micropython_ti_get_builtin_argument_name(
  ...
  return &micropython_ti_builtin_name_pool[builtin_argument->name_offset];
}
...
micropython_ti_collect_builtin_methods_matching_partial_method_name(
  ...
  const uint8_t *partial_method_name,
  size_t partial_method_name_length,
  const MicroPythonTiBuiltinMethod **output_methods,
  int output_capacity
) {
...
const char *
micropython_ti_get_builtin_method_name(
  const MicroPythonTiBuiltinMethod *builtin_method
) {

  return &micropython_ti_builtin_name_pool[builtin_method->name_offset];
}

const char *
micropython_ti_get_builtin_signature(
  const MicroPythonTiBuiltinMethod *builtin_method
) {

  return &micropython_ti_builtin_signature_pool[
    builtin_method->signature_offset
  ];
}

const char *
micropython_ti_get_builtin_document(
  const MicroPythonTiBuiltinMethod *builtin_method
) {

  return &micropython_ti_builtin_document_pool[builtin_method->document_offset];
}

const char *
micropython_ti_get_builtin_class_name(uint8_t class_id) {
  if (
    class_id == MICROPYTHON_TI_CLASS_NONE ||
    class_id >= micropython_ti_builtin_class_count
  )
  ...
  return &micropython_ti_builtin_name_pool[
    micropython_ti_builtin_classes[class_id].name_offset
  ];
}
...
collect_builtin_union_class_ids(
  ...
  if (
    ...
    builtin_union_index >= micropython_ti_builtin_union_count
  ) {
  ...
  const MicroPythonTiBuiltinUnion *builtin_union =
    &micropython_ti_builtin_unions[builtin_union_index];
...
micropython_ti_get_builtin_argument_classes(
  ...
  if (builtin_argument->class_identifier == MICROPYTHON_TI_CLASS_NONE)
    return 0;
...
micropython_ti_get_builtin_return_classes(
  ...
  if (builtin_method->return_class_identifier == MICROPYTHON_TI_CLASS_NONE)
    return 0;
...
micropython_ti_get_builtin_return_array_variant_classes(
  ...
  if (
    builtin_method->return_array_variant_class_identifier ==
